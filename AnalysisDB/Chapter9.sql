...
FROM 'C:\Users\User\Documents\Analysis_PostgreSQL\MyWork\AnalysisDB\MPI_Directory_by_Establishment_Name.csv'
...
--SELECT meat_poultry_egg_inspect.city, meat_poultry_egg_inspect.zip
--FROM meat_poultry_egg_inspect 
-- WHERE (meat_poultry_egg_inspect.city = 'Blaine' 
-- AND meat_poultry_egg_inspect.zip = '55449');
...
-- In order to clean up this, set parameters for values
...
-- Comparing original vs. back - up table if it's the same
SELECT 
...
-- Technique useful if working with large database
...
-- zip is varchar hence why '00' would show

UPDATE meat_poultry_egg_inspect
SET zip = '00' || zip --concatenate
...
COPY state_regions
FROM 'C:\Users\User\Documents\Analysis_PostgreSQL\MyWork\AnalysisDB\state_regions.csv'
WITH (FORMAT CSV, HEADER, DELIMITER ',');
...
WHERE EXISTS (SELECT state_regions.region -- where this condition is met, set to inspection date
              FROM state_regions
...
START TRANSACTION;
-- Safe guarding database - undo button.
-- Likened to git revert 
-- Transaction ended as soon as a new one has started
...
-- if all changes made to back up and wanting to make it original, perform this step
